...
const SEARCH_GUIDANCE = "Do not call web_search or web_fetch. Those DSH tools are not available on this route. Cloud Code Assist v1internal cannot mix built-in googleSearch with function tools. Use generate_image when the user asks for an image.";
...
function ccaFunctionDeclarations(tools, image) {
	const kept = filterDshWebTools(tools ?? []).filter((tool) => tool.name !== GENERATE_IMAGE_TOOL).map((tool) => ({
		name: tool.name,
...
function functionsFor(options, nativeImage) {
...
function buildCcaBody(projectId, input, envelope) {
	return input.kind === "chat" ? buildChatBody(projectId, input, envelope) : buildImageBody(projectId, input);
}
//#endregion
//#region src/sse.ts
...
var CcaClient = class {
	...
	async *generate(oauth, input, signal) {
		const body = input.kind === "image" ? buildImageBody(oauth.projectId, input, this.now()) : buildChatBody(oauth.projectId, input, advanceEnvelope(this.session, input.model, this.now(), this.lastExecutionId));

...
function sanitizeGeminiSchema(raw) {
	...
	if (Array.isArray(source.enum)) {
		const values = source.enum.filter((entry) => typeof entry === "string" || typeof entry === "number" || typeof entry === "boolean");
...
const SEARCH_GUIDANCE = "This turn already includes Google Search grounding. Do not call web_search or web_fetch — those DSH tools are not available on this route. Use generate_image when the user asks for an image.";
...
function functionTools(functions) {
	...
	return [{ googleSearch: {} }, { functionDeclarations: functions.map((tool) => ({
		name: tool.name,
		description: tool.description,
...
function buildImageBody(projectId, input, envelope) {
	const parts = [];
	...
	const imageConfig = {};
	if (input.aspectRatio !== void 0) imageConfig.aspectRatio = input.aspectRatio;
	...
	return wrap(projectId, IMAGE_MODEL, {
		...
		generationConfig: {
			...
			imageConfig,
			candidateCount: 1,
			maxOutputTokens: WIRE_PROFILES["gemini-3-pro-image"]?.maxOutputTokens ?? 65536
		},
		safetySettings: IMAGE_SAFETY
	}, envelope);
}
function buildCcaBody(projectId, input, envelope) {
	return input.kind === "chat" ? buildChatBody(projectId, input, envelope) : buildImageBody(projectId, input, envelope);
}
//#endregion
//#region src/sse.ts
		...
		for (let index = 0; index < endpoints.length; index += 1) {
			...
			try {
				...
				if (!response.ok) {
					...
					const error = /* @__PURE__ */ new Error(`CCA ${response.status}: ${text.slice(0, 800)}`);

...
var AntigravityAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		...
		if (await this.session.refreshIfNeeded() === void 0) throw new LlmError("Antigravity is not connected. Open Settings and sign in.", "MISSING_CREDENTIAL");
	...
	async *emitChat(options, input, state, watchdog) {
	...
	async collectSearch(options, query, watchdog) {
		const oauth = await this.session.refreshIfNeeded();
		if (oauth === void 0) throw new LlmError("Antigravity is not connected. Open Settings and sign in.", "MISSING_CREDENTIAL");
		if (!isPublicModelId(options.model)) throw new LlmError(`unknown Antigravity model "${options.model}"`, "UNKNOWN_MODEL");
		const effort = effortOf(options);
...
var AntigravitySession = class {
	...
	async signIn() {
		...
		if (this.pendingUrl !== void 0 && this.account.status === "signing-in") return { url: this.pendingUrl };
		await this.operation?.catch(() => void 0);
		if (this.pendingUrl !== void 0 && this.account.status === "signing-in") return { url: this.pendingUrl };
	...
	async finishCode(code, signal) {
		...
		try {
			...
			this.account = stored.status === "authorized" ? stored : {
				status: "error",

...
const SEARCH_ONLY_GUIDANCE = "Use search_web for news and public-web facts. Do not call web_search, web_fetch, or generate_image. This route has no image generation. Cloud Code Assist v1internal cannot mix built-in googleSearch with function tools, so search_web runs as a separate googleSearch-only request.";
...
function wantsNativeImage(text) {
...
function ccaFunctionDeclarations(tools, image, search = true) {
	const kept = filterDshWebTools(tools ?? []).filter((tool) => tool.name !== "generate_image" && tool.name !== "search_web").map((tool) => ({
		name: tool.name,
...
function maskDshWebAssembly(assembly, image = false) {
	return {
		...assembly,
...
function parseGenerateImageArgs(args) {
...
function functionsFor(options, nativeImage, nativeSearch) {
	if (options.purpose === "compaction" || options.purpose === "session-title") return [];
	const latest = latestUserText(options.messages);
	return ccaFunctionDeclarations(options.tools, nativeImage && wantsNativeImage(latest), nativeSearch);
}
		...
		try {
			for await (const event of events) {
				...
				if (event.type === "functionCall") {
					yield* closeThought();
					yield* closeText();
					if (event.name === "generate_image" && this.options.nativeImage) {
			...
			if (pendingImages.length > 0) try {
				const oauth = await this.session.refreshIfNeeded();
				if (oauth === void 0) throw new LlmError("Antigravity is not connected. Open Settings and sign in.", "MISSING_CREDENTIAL");
				for (const image of pendingImages) for await (const event of this.session.cca.image(oauth, {
					kind: "image",
					...image
				}, options.signal)) {
					if (event.type === "inlineImage") {
...
const WIRE_PROFILES = {
	...
	"gemini-3-pro-image": { maxOutputTokens: 65536 },
	"gemini-3.1-flash-image": { maxOutputTokens: 65536 },
	"gemini-3-pro-image-preview": { maxOutputTokens: 65536 }
};
...
function imageRequestId(now = Date.now()) {
	return `agent/${randomUUID()}/${now}/${randomUUID()}/2`;
}
function buildImageBody(projectId, input, now = Date.now()) {
	const parts = [];
	for (const image of input.inputImages ?? []) parts.push({ inlineData: {
		mimeType: image.mimeType,
		data: image.data
	} });
	parts.push({ text: input.prompt });
	const imageConfig = { aspectRatio: input.aspectRatio ?? "1:1" };
	if (input.imageSize !== void 0) imageConfig.imageSize = input.imageSize;
	return {
		project: projectId,
		...
		request: {
			...
			systemInstruction: systemInstruction(IMAGE_SYSTEM_INSTRUCTION, void 0),
			generationConfig: {
...
function isImageFallbackStatus(error) {
	return error instanceof CcaHttpError && (error.status === 404 || error.status === 403);
}
function classifyStatus(status, kind) {
...
function imageModels(preferred) {
	const first = preferred ?? "gemini-3-pro-image";
...
var CcaClient = class {
	...
	async *image(oauth, input, signal) {
		let lastError;
		for (const model of imageModels(input.model)) try {
			yield* this.generate(oauth, {
				...input,
				model
			}, signal);
			return;
		...
		throw lastError ?? /* @__PURE__ */ new Error("CCA image request failed");
	}
	endpoints() {
	...
	async *generate(oauth, input, signal) {
		const body = input.kind === "image" ? buildImageBody(oauth.projectId, input, this.now()) : input.kind === "search" ? buildSearchBody(oauth.projectId, input, this.now()) : buildChatBody(oauth.projectId, input, advanceEnvelope(this.session, input.model, this.now(), this.lastExecutionId));
		...
		for (let index = 0; index < endpoints.length; index += 1) {
			...
			try {
				...
				if (!response.ok) {
					...
					if (classifyStatus(response.status, input.kind) === "retry" && index < endpoints.length - 1) {
						lastError = error;
						continue;
					}
					throw error;
...
function apply(ctx, config) {
	...
	if (config.nativeTools !== false) ctx.inject(["systemPrompt"], (promptCtx) => {
		promptCtx.on("system-prompt/assemble", async (_assembly, context, next) => {
			...
			return context.agent?.options?.provider === "agy-google-antigravity" ? maskDshWebAssembly(assembled, config.nativeImage === true) : assembled;
		});
	});
}
//#endregion
